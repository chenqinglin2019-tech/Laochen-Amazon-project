...
class BackendLauncherTests(unittest.TestCase):
    ...
    def invoke(self, command="validate", **kwargs):
        ...
        options.update({"keywords_file" if command == "qa" else "listing_file": self.source})
    ...
    def test_full_qa_files_are_redacted_before_publication(self):
        for command in ("qa",):
    ...
    def test_failure_conditions_never_report_success(self):
        cases = [("validate", {"ok": True}, 0, "backend_rejected"),
                 ...
                 ("qa", {"ok": False}, 0, "backend_rejected"),
                 ("qa", {"errors": ["failed question request"]}, 0, "backend_rejected"),
    ...
    def test_missing_full_output_does_not_fall_back_to_stdout_or_replace_old_file(self):
        ...
        with patch.object(backend.subprocess, "run", return_value=subprocess.CompletedProcess([], 0, '{"keywords":[]}', "")):
            result = self.invoke("qa", output=self.output)
    ...
    def test_default_config_is_relative_to_skill_and_platform_selection_is_explicit(self):
        ...
        for system, machine, suffix in (("Darwin", "arm64", "darwin-arm64"), ("Darwin", "x86_64", "darwin-amd64"),
            ...
            with patch.object(backend.platform, "system", return_value=system), patch.object(backend.platform, "machine", return_value=machine):
                self.assertEqual(backend.select_cli().name, "laochen-cli-v2-" + suffix)
                self.assertTrue(backend.select_cli().is_file())
    ...
    def test_cloud_expansion_and_non_us_qa_stop_before_auth(self):
        with patch.object(backend, "load_config") as config, patch.object(backend.subprocess, "run") as runner:
            self.assertEqual(self.invoke("expand")["error_code"], "invalid_arguments")
            for site in backend.SITES:
                if site != "US":
                    self.assertEqual(self.invoke("qa", site=site)["error_code"], "qa_us_only")
            with contextlib.redirect_stderr(io.StringIO()), self.assertRaises(SystemExit) as done:
                backend.main(["expand", "--site", "US", "--asins", "B000000001"])
            self.assertEqual(done.exception.code, 2)
            config.assert_not_called()
            runner.assert_not_called()

    def test_non_us_validation_still_uses_same_authentication(self):
        for site in backend.SITES:
            with patch.object(backend.subprocess, "run", side_effect=self.completed({"ok": True, "errors": []})) as runner:
                self.assertNotIn("failure_reason", self.invoke(site=site))
                self.assertEqual(runner.call_args.kwargs["env"]["LAOCHEN_BACKEND_TOKEN"], CONFIG["backend_token"])
                args = runner.call_args.args[0]
                self.assertEqual(args[args.index("--site") + 1], site)
    ...
    def test_private_raw_file_is_cleaned_after_timeout(self):
        ...
        with patch.object(backend.subprocess, "run", side_effect=interrupted):
            result = self.invoke("qa", output=self.output, timeout=1)
...
''', encoding="utf-8")
        for command in ("qa", "validate"):
            ...
            args += ["--keywords-file" if command == "qa" else "--listing-file", str(self.source)]
