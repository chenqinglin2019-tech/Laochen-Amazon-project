...
首次集中完成整套准确且已批准的文案、素材和版式规划，预查容量与依据。主图／无字图用 `none`；普通场景海报、A+、卖点及拼版默认 `local_overlay`，先生成无字底图，再按项目契约排字。新版 `prompt_profile: images_2_5_v1` 的普通摄影海报可选择 `model_native_reason.kind: native_poster`，用一个标题和可选简短非数值正文，事实性卖点绑定 `job.claim_ids`。尺寸、数值规格、步骤、FAQ、必要限制及精确营销品牌／Logo整图保留本地排字；装饰性艺术字理由及 3D 嵌字仍受短标题规则约束，3D 另填 `embedding_decision`。native 只绘制 `job.copy` 中已批准内容；两条文字路线不能重复叠字，也不能让模型扩写卖点。

提示规划按 [联合 imagegen 提示契约](imagegen-prompt-profile.md) 在编译／指纹绑定前完成：参考角色与实际附件顺序一致，当前画幅只保留一个确定构图，四项锁区分不可变产品事实和可变摄影表现。实际派发不再二次增强已绑定提示。缺少 prompt_profile 或显式legacy的旧图保留原提示字节与生成指纹；本地排版／QA仍按现有源码哈希检查复核，不因此重生底图或重写旧审核哈希，升级只作用于明确选择的图。
