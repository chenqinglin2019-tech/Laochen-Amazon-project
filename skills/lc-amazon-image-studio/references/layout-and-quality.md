...
先独立选择 `text_mode`：`none` 无营销文字，`local_overlay` 默认用本地精确排版，可选局部浅浮雕仍留在此路线。新版 `prompt_profile: images_2_5_v1` 的普通摄影海报可用 `model_native_reason.kind: native_poster`，含一个标题与可选简短非数值正文，事实性卖点绑定 `job.claim_ids`；尺寸、数值规格、步骤、FAQ、必要限制及精确营销品牌／Logo整图本地排字。装饰艺术字和3D嵌字仍遵守短标题规则，缺 profile 的旧图保留原语义。商品生成的 `render_mode` 仍由清晰度、视角与证据决定；`model_native` 不用于主图或 `pixel_composite`，不能混入本地文字、图标和 panels 造成重复叠字。

本地版式属于本 skill：HTML/CSS/SVG、随包字体和图标经 Playwright／Chromium 渲染，Pillow 合成及导出。就绪任务在同批复用浏览器，资源本地加载；文案按文本转义，不执行 HTML 或脚本。native 路线仅在成品阶段跳过排字与对应字体加载，预检仍真实测量容量，不跳过最终原尺寸、360 预览、产品及设计审阅。
...
- local路线缺字、溢出、碰撞进入本地布局修复；颜色、文字或局部背景修正只重排该图。局部浅浮雕只修相关效果/排版，失效时恢复完整平面字；native错字或设计失败进入该图模型修复，不重复叠本地字。改变生成构图才重生相关底图，按受控次数停止无效试错。
...
native 不受本地字体／字形对比度自动检查覆盖；现有 `reviews.model_text_review.notes` 须保存实际最终 JPG 的路径／哈希、字形核心取样方法、最低4.5:1数值证据及360预览标题18px／正文12px的测量依据。编码后需要补充时用 `review-prepare --force` 取得新包再提交，不改旧已提交包；最终编码字节变化须重测，不能把整框亮度或目测当成达标。无法验证这些既有要求时改用local_overlay与无字底图，不在native图上重复叠字。
