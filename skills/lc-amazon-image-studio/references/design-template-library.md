...
Put background, camera, lighting, scale relationships, and generative composition in `generation` and the prompt. Put typography and exact local text placement in `layout`; place explanatory fixed-style/adaptation/avoid notes in design guidance. The compiler separates these dependencies. For `prompt_profile: images_2_5_v1`, apply the [official imagegen integration](imagegen-prompt-profile.md) before fingerprint binding: resolve the active canvas and explicit overrides into one composition, remove only complete duplicate prompt fragments, and send the bound prompt unchanged. Preserve unique design requirements and all adopted snapshots. Native decorative lettering retains its short-title rules; ordinary `native_poster` copy follows the profile's non-numerical poster rules. Approved copy comes only from the project, never reference marketing text.
