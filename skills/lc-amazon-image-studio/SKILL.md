...
This gate only verifies whether the current user may use this skill. It does not replace any product facts, competitor references, or image-generation decisions.
...
Then run the selected auth tool from the skill root:

```bash
./tools/bin/lc-auth-check-linux-amd64
./tools/bin/lc-auth-check-darwin-arm64
./tools/bin/lc-auth-check-darwin-amd64
```

On Windows, use:

```powershell
.\tools\bin\lc-auth-check-windows-amd64.exe
```

Never print or reveal the full key.

If auth fails, if `backend_token` is missing, or if the auth binary is unavailable, stop immediately. Do not inspect images, do not inspect competitors, do not plan, do not write prompts, and do not generate images. Use only this safe message:
