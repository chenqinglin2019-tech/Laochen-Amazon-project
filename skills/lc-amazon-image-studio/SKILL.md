...
- 生成或恢复：读 [运行流程](references/runtime-pipeline.md)、[设计与性能契约](references/v5-design-and-performance.md)。同轮后续 QA／交付直接使用已读取的对应阶段，不重复全套阅读或准备。本地脚本不调用模型，默认使用内置 image_gen。
...
正常起点只运行一次 `plan --json`，它已包含 prepare：以仅测量模式检查真实字体、文字容量、区域和保护区，不生成排版截图或审核包。保留独立 prepare 接口；输入改变或恢复状态不明时重新 plan。派发锁内仅复核当前来源、递归证据与任务绑定，不制作预览或重新排版；缺准备或绑定过期必须重新规划。

新项目采用 `scheduler_policy={version:1,mode:adaptive,max_concurrency:4}`，`concurrency` 保存实时档位，健康状态沿用 network_health。仅派发一个最高风险可执行锚点；真实 QA 通过后从 2 开始，同档位连续两个新 attempt 成功入库升一档，最高 4。429 降 1，单次超时降一档，连续两次超时降 1；退避后 60 秒不升速，遵守工具明确给出的重试等待时间。重复回调不重复计数，退避前派发的迟到成功不参与升速；降档不取消在途任务。产品生成与局部标题编辑共用容量；工具能力较低时用 `plan --tool-capacity` 登记实际上限。旧项目无策略维持原行为。

审核、排版和导出独立列为本地动作，不占模型名额；仍须满足来源及锚图检查。transition generating 获得 attempt 与提示指纹；真实开始记录 attempt-event，返回立即 `ingest --tool-returned-at <真实返回epoch>` 绑定绝对产物路径和返回时间，也兼容分步 tool_returned 事件。重复同绑定幂等；旧 attempt／旧提示／不同产物冲突拒绝。禁止手改状态或哈希伪装完成。

ingest、review-submit 返回最新可执行任务及提示文件／参考路径。预读提示与素材后，将 transition、tool_started 与实际生图调用紧接执行。有结果立即入库，有可执行任务立即补位，不等同批其他图；返回后先 ingest 再审图，不能把代理准备／等待记成模型时间。
...
完整 finalize 更新总览，再 `deliver --json` 验证、整理并返回 `output_dir`、`images`、`image_count`。新项目默认 compact_jpg：主图、副图、A+ 保留有序文件名平铺于 `final/`，只交付通过 QA 的最终 JPG；保持上传尺寸、质量 92、4:4:4，细节不满意的单图用 95。原始来源、采用底图、Manifest、文案／布局及真实审核／计时记录留在项目工作目录。总览、报告、预览不混入成品文件夹；最终回复提供 output_dir 的文件夹入口，不生成 ZIP 或 HTML。

通过交付检查后持久保存证据绑定，再清理已登记的自有预览、对比缓存及未采用候选；整理前后均核验，缓存可重建，输入或成品改变仍失效。无变化再次交付不重编码、不重复复制成品。旧项目不自动迁移；明确 deliver 时将分散的已审成品按清单汇集到新的版本文件夹，逐文件验哈希、不覆盖历史，缺 compact_jpg 的旧编码行为不变。全部必需图通过才声明整套完成，不默认上传 Amazon。本次本地实现变化仅触发相关本地复核，不改生成提示／版本指纹，不改历史哈希继承审核。
