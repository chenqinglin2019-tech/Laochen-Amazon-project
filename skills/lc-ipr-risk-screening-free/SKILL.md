...
新任务默认 `retrieval_workflow_revision=api-first-v1` 与 `completion_policy_revision=necessary-work-v2`：优先使用已授权且可用的 API，完成有界发现、分流、必要比较与独立双审。官方核验受阻时可交付来源证据报告，逐项标明未经官方核验；无依据范围的原始审阅保持待评，报告风险按下述新评级修订输出。执行前读取 [API 优先检索契约](references/api-first.md)及[有界取证交付规则](references/workflow-correction.md#有界取证交付necessary-work-v2)。鉴权完全遵循原包冻结基准；发现证据不提升为官方覆盖或现行效力证明。历史任务保留原修订。

新任务使用 `2.4-free` / `automation-first-v1`、`assessment_policy=evidence-estimate-v1` 与 `assessment_revision=partial-evidence-v1`，并启用 `screening_revision=recall-integrity-v1` 与 `decision_workflow_revision=scenario-triage-v1`。先固定情景，再全量轻量分流，仅对入选候选深入核验。完整任务沿用原有证据评级；不完整任务在不改变真实完成状态、原始双审 pending/null 或权属 unknown 的前提下，于派生评级/报告层保留有证据支持的中／高／极高，其他当前适用项标为低且所有置信度为低。有效权利检索无命中仍为低，但必须醒目标明“未排除侵权风险”。历史无该修订的任务保留原修订与产物，重评必须创建新目录。先执行 [INSTRUCTIONS.md](INSTRUCTIONS.md) 的业务云端鉴权；该鉴权检查 Token 和本 Skill 使用权限，不扣业务积分。开发、审计、离线测试或恢复 Skill 文件不是执行商品排查。
...
新任务以 `next_work` 为唯一待办入口。调度、查询和恢复仍沿用 v2；默认 `--mode auto`，严格完整时发布 `final`，有真实受阻依据时可发布不完整的 `evidence`。新评级修订仅在报告门禁接受绑定具体查询的技术错误记录，不改原待办状态、重试次数或执行权限。未读、未审、未执行且无受阻记录的工作及证据损坏仍阻断；停止说明不替代工作。`stage` 保留显式临时交付用途，旧修订保持原门禁。
...
- `partial-evidence-v1` 只作用于评级、报告及独立校验：不得修改鉴权、API 配置或调用、浏览器路线、采集、查询计划执行及恢复逻辑。产品身份确认但有效权利检索为零时仍输出低风险／低置信度，醒目标注“有效检索为零；本结论为规则兜底，未排除侵权风险”。
...
- 最终风险仅为 `极高／高／中／低／极低`，置信度为 `高／中／低`；事实状态仍可未知。未带 `assessment_revision=partial-evidence-v1` 的历史任务中，待评是执行状态而非第六级：无有效相关检索、也无具体排除依据时不生成最终低风险。新修订的不完整报告仅在派生层以 `risk_basis=policy_fallback` 标出当前适用项低风险，绝不把未执行、失败、截断或未知提交写成已排除；已取得的具体风险不因其他模块缺口被清空，缺口本身也不自动判中。
- 总评取同一主情景中主审确认的最高适用当前风险；不同销售情景分别汇总，不平均、不重复计算同申请文献或转载来源。完整任务及历史策略的置信度取决于驱动证据链；新修订不完整时统一低置信度，低风险总评标明规则兜底，不能把局部排除当作整体排除证据。未来申请及维权记录单列。
...
7. 新策略按 [risk-estimate-rules.md](references/risk-estimate-rules.md) 比较、定级并说明正反证据、推论、置信度及升降级条件；`partial-evidence-v1` 仅为不完整报告增加可追溯的派生评级，不改双审原值、权属事实或完成状态；无策略字段的历史任务才使用 [risk-rules.md](references/risk-rules.md)。专利逐权利要求要素；外观核对实际必要视图；商标核对标识、商品服务与使用。运行 `record_asset_provenance.py --list-work`，完成图形标样比较、适用版权及商业外观公开调查，再通过同一记录器留存分步骤证据；API/浏览器均不执行这些 Agent 待办，不能跳过。调查完成与权属事实未知分别记录，登记库零结果不排除版权。
...
9. 用 `publish_report.py --mode auto` 在新输出目录定稿并独立验收五项报告。v2 的 `evidence` 表示本轮证据交付完成，不是官方全面核验完成；逐项说明来源、时间、支持事实、未核验事项及影响。旧 v1 仍用 final/stage。独立 finalize/build/validate 共用门禁，真实待补充、截断和覆盖缺口保持可见；缺口不填入反证栏。真实页面验收按 [浏览器契约](references/browser-product-capture.md) 与 [召回验收](references/recall-acceptance.md) 执行。
...
python scripts/publish_report.py --task-dir /absolute/run --first-review /absolute/run/first-review.json --second-review /absolute/run/second-review.json --adjudication /absolute/run/adjudication.json --assessment-revision partial-evidence-v1 --mode auto --output-dir /absolute/new-report
```

2.4 历史输入显式重评及补充证据命令见 [evidence-schema.md](references/evidence-schema.md)；必须以 `--assessment-revision partial-evidence-v1` 和新 `--output-dir` 生成新摘要、盲双审及报告，不能改旧任务、原始审阅或既有产物。未带该修订的历史分支保持原行为；2.1/2.2/2.3 不通过修改版本号迁移。数据契约与审阅字段见该文件，商标和版权专项见 [trademark-copyright.md](references/trademark-copyright.md)。`report-data.json` 是 HTML、Markdown、CSV 的统一视图模型；五项产物均保留输入摘要、证据哈希和离线可读性。未查清部分不得用文字包装成清白证明。
