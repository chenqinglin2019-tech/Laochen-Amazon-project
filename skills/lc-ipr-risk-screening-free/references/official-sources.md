...
EPO OPS、通过免费账户门禁的 SerpApi 可提高公开专利发现；Serper 在 2.4 因尚不能独立确认账户免费权益而暂停计量查询。仍须分别解决权利要求、设计专利完整图纸和当期状态。上述缺口不会因 Google Patents 的状态摘要或某一诉讼库无命中而消失。
...
SerpApi 与 Signa 经账户门禁后可使用免费额度发现。Serper 虽提供注册免费额度，但 2.4 缺少已验收的账户免费权益取证，固定在计量请求前停止；不能把注册赠额推定为任意现有 Key 均可零费用执行，细节见 providers.md。两个 Google 检索代理不算两个独立数据库；增加语言、结构、分类、权利人、图片维度才可能扩大召回。发现层内容不能伪称官方登记或证明权利现行有效。新策略可将真实线索作为带假设、低置信度的预判依据；不把线索转换为已核实事实。无策略字段的历史任务仍按旧正式评级门禁执行。
