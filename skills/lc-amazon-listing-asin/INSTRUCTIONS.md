...
用户输入目标 site、竞品 asins（1–20 个）、本品图片或文字资料，以及可选品牌、必保词、禁用词、子体 SKU/ASIN 与变体信息。
...
- 竞品 ASIN 整组扩词一次；竞品不是用户的子体，Listing 不代表竞品共性。只生成用户明确提供的组合，不补齐颜色×尺码组合。
...
- 核心品名在扩词结果中缺席或流量未知也不能删除。后台类目枚举不替代消费者品名；不要拆散完整产品短语。
...
- expand、qa、validate 由统一入口调用 tools/bin 中当前平台的 laochen-cli，不直接请求第三方采集服务。listing_quality.py backend 内部也调用该入口。核验规则出处可以只读访问官方资料。
...
- expand/qa 原始结果先落到私有临时目录，解析并脱敏后原子写入 --output；validate 响应使用同一脱敏逻辑。终端只显示简短状态/错误类别或脱敏 JSON，不透传原始 stdout/stderr。任务文件保留原有数据结构，凭据及敏感字段替换为 [REDACTED]。
- 同时检查启动脚本退出码和保存的业务结果；有文件不等于成功，部分 ASIN 失败仍需逐项报告。无有效 JSON 时保留旧目标文件，不得当作本次结果继续使用。expand/qa 默认沿用原 CLI 内置轮询时限，validate 保留 120 秒超时，可用 --timeout 指定额外时限；失败/超时不自动重试，先核对任务状态。
...
先读 knowledge/distilled/product_facts_rules.yaml。保存 00_product_input.json：用户原始描述、站点、竞品 ASIN、品牌、约束、图片定位及明确子体清单。按 data_contract 生成 01_product_profile.json。

保留 category、audience、materials、functions、scenes、specs、limitations、pain_points；未知用 null/unknown，不把“未证实防水”写成已证实不防水。量化规格须有用户文字、规格图标注或产品资料支持。新增 product_identity、facts、measurements 和 images，确定核心品名后不让扩词重新决定产品身份。
...
```text
python3 SKILL/scripts/backend_cli.py expand --asins ASIN1,ASIN2 --site SITE --output RUN/02_kw_raw.json
```

必须使用 --output 保存完整 keywords/raw 响应，不依赖可能截断的 stdout。CLI 自动轮询，耗时随数量和队列变化。检查成功 ASIN 和错误；部分失败不描述为全组成功，不伪造缺失流量。
...
新任务不生成 03_keyword_review.json，也不执行 prepare --review；不以抽样或风险词复核替换成另一道强制筛词复核步骤。旧复核文件仅作为历史资料留存，不参与导出、完成条件或当前指纹。复用旧任务时，若 reason_code/reason/evidence 仍描述旧分类，先依据真实资料更新为当前 decision 的理由，并在 resolution 记录修正；无需重跑扩词或补做第二轮剔除词复核。
