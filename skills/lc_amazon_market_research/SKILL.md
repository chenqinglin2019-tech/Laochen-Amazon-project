...
本 skill 用本地 SellerSprite / 卖家精灵 Excel 导出文件生成市场主报告。执行前必须联网完成一次用户访问校验，该校验不扣积分；校验通过后，主报告的数据处理和生成全部在本地执行。主报告完成后，可按固定流程追加“头部品牌/卖家外部调研”。外部调研属于公开信息推断，不是尽调结论。
...
3. 读取 `config.json`，调用所选 CLI 的 `auth-check`。校验失败、Token 缺失、账户停用、余额不足或校验服务不可用时立即停止，不读取输入、不创建项目目录、不运行任何分析命令。
4. 校验通过后，让用户提供本次调研 Excel 所在文件夹，或四类 Excel 文件。
5. 调用所选 CLI 的 `inspect-inputs` 识别文件类型、站点、核心关键词和主类目。
...
9. 如用户需要头部品牌/卖家外部调研，调用所选 CLI 的 `external-targets` 生成 Top 品牌/卖家调研对象，联网搜索后写入固定 JSON，再调用 `attach-external` 追加结果。
10. 让用户直接打开 `market_research/市场调研报告看板.html`，不要启动本地服务。
11. 按 `INSTRUCTIONS.md` 固定顺序解读，不新增文档未定义的指标或互动流程。

## 资源

- `references/input_contract.md`：输入 Excel 文件和字段要求。
- `references/calculation_spec.md`：指标、透视和规则结论口径。
- `config.json`：用户访问校验配置；`backend_token` 由交付方或用户填写。
