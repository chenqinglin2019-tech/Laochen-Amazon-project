...
7. **过程留痕必须保留**：清洗、剔除、覆盖率、公式口径都要能在辅助 Excel 或 JSON 中查到。
8. **不要泄露无关本地环境信息或完整 Token**。
9. **写文件必须用 UTF-8 编码**。
10. **外部调研只能作为追加结果**：它属于公开信息推断，不是尽调结论；不可推翻结构化市场数据，只能辅助解释竞争强度。
...
本 skill 已使用 Go CLI 打包，不依赖 Python 或本机脚本环境。执行命令前先按用户机器选择一个 CLI，后文用 `<MR_CLI>` 代指：
...
先调用 CLI 生成全量相关性工作区，不要让 agent 临场手写脚本读 Excel：
...
根据平台选择命令，使用上方 `<MR_CLI>`。Windows 使用 `tools\bin\market-research-windows-amd64.exe`。
...
- 除执行前的一次用户访问校验外，主报告不要调用线上业务后端。
- 不要在访问校验失败后读取输入、创建目录或继续任何报告流程。
