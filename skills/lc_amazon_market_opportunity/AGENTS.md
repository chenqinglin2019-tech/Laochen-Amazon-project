...
- 优先使用项目根目录；不要让用户复制 `market_research/` 子目录路径。
- 支持继承 `US`、`UK`、`DE`、`FR`、`JP`、`AU`、`CA`、`IT`、`ES`、`MX`。站点以项目 manifest 为唯一依据；本段不重新询问、不默认 US、不允许改站点。旧项目中的 `GB` 只规范为 `UK`。
- `inspect-report` 返回 `listing_language` 后，维度确认、归一化和逐 listing 打标必须理解该语言。原始关键词、标题、参数和 evidence 保留源语言，`display_values` 仍输出中文。不得因为不是英语就直接标为不可识别。
- 固定停止话术：`商品机会深挖依赖上一段市场主报告结果；当前上下文没有可用的 market_project 项目目录，本轮不继续执行。请先完成市场主报告后，在同一上下文里继续。`
- 关键词词根只能通过 `market-opportunity fetch-roots` 调我们的统一后端；不得直连任何外部词根服务。agent 不处理、不询问、不记录外部供应商接口、账号或凭据。
- 不要读取、打印或在回复里复述 `config.json` / 环境变量里的后端凭据；只调用 CLI。
- 如果 `fetch-roots` 明确返回缺少后端 token，立即停止，只说明“商品机会深挖访问配置缺失，请补齐该 Skill 的用户访问 Token 后继续”；不要询问供应商 token，不要绕过词根步骤。
...
- 每个 listing 必须一次性判断全部维度，不要按维度拆成多轮。
- 优先用标题、SKU/详细参数；文本无法判断的维度才看图片或链接。
- 无法稳定判断填 `不可识别`，不要猜。
- 不得只套用英文关键词表处理 DE、FR、JP、IT、ES、MX 等站点；应先结合目标语言标题、参数和上下文做语义判断，确实存在歧义或证据不足时才标为 `不可识别`。
...
- `<selected-cli> fetch-roots --report-dir ... --output-dir ...`：通过统一后端获取关键词词根。
